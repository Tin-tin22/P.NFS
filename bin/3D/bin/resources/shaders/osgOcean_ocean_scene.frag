...
uniform bool osgOcean_EnableGlow; //BAWE-20110805
...
const float glowAlphaValue = 0.5; //BAWE: The alpha value that indicates glowing
...
void main(void)
{
    ...
    if ( textureColor.a < 0.51 && textureColor.a > 0.49 )
    {
        isGlowing = true;
    }

    if (osgOcean_EnableGlow ) 
    {
        if ( isGlowing )
        {
            gl_FragData[1] = vec4(textureColor.rgb, 1.0);
    ...
    if(osgOcean_EyeUnderwater && vWorldHeight < osgOcean_WaterHeight+2.0 )
    {
        ...
        if ( isGlowing )
        {
            gl_FragData[0] = vec4(textureColor.rgb, 0.2);
