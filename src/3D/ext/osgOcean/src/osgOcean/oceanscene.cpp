...
OceanScene::OceanScene( void )
    ...
    ,_enableDOF                  ( false )
    ,_enableGlare                ( false )
    ,_enableGlow                 ( false ) // BAWE-20110728: GLOW_SHADER
    ...
    ,_glowMask                   ( 0x40 )  // 64 // BAWE-20110728: GLOW_SHADER. NOT USED
    ,_lightID                    ( 0 )
    ,_dofNear                    ( 0.f )
    ,_dofFar                     ( 160.f )
    ,_dofFocus                   ( 30.f )
...
OceanScene::OceanScene( OceanTechnique* technique )
    ...
    ,_enableUnderwaterScattering ( false )
    ,_enableDefaultShader        ( true )
    ,_enableGlow                 ( false ) // BAWE-20110801: GLOW_SHADER
    ,_reflectionTexSize          ( 512,512 )
    ,_refractionTexSize          ( 512,512 )
    ,_screenDims                 ( 1024,768 )
    ,_sunDirection               ( 0,0,-1 )
...
OceanScene::OceanScene( const OceanScene& copy, const osg::CopyOp& copyop )
    ...
    ,_enableGlow                 ( copy._enableGlow ) // BAWE-20110801: GLOW_SHADER
    ...
    ,_glowMask                   ( copy._glowMask )  // 64 // BAWE-20110728: GLOW_SHADER. NOT USED
    ...
    ,_glowPasses                 ( copy._glowPasses ) // BAWE-20110803: GLOW_SHADER
    ...
    ,_glowStateSet               ( copy._glowStateSet ) // BAWE-20110803: GLOW_SHADER
...
void OceanScene::init( void )
{
    ...
    if( _oceanSurface.valid() )
    {
        ...
        _globalStateSet->addUniform( new osg::Uniform("osgOcean_EnableGlow", _enableGlow ) ); // BAWE-20110805: GLOW_SHADER
        ...
        if( _enableGlare )
        {
            ...
            //_glarePasses.push_back( glareCombinerPass(luminanceTexture, streakBuffer2, streakBuffer4, streakBuffer6, streakBuffer8 ) );
        }
        ...
        if( _enableGlow )
        {
            ...
            _glowStateSet = new osg::StateSet;
            _glowStateSet->addUniform( new osg::Uniform("osgOcean_EnableGlow", _enableGlow ) );
            ...
            //osg::TextureRectangle* fullScreenGlow = createTextureRectangle( _screenDims, GL_RGBA );
            osg::TextureRectangle* fullScreenGlow = createTextureRectangle( _screenDims, GL_RGBA );

            osg::Camera* fullPass = multipleRenderTargetPass( fullScreenTexture,   osg::Camera::COLOR_BUFFER0,
                                                              fullScreenGlow, osg::Camera::COLOR_BUFFER1 );

            fullPass->setCullCallback( new PrerenderCameraCullCallback(this) );
            fullPass->setStateSet(_glowStateSet.get());
            _glowPasses.push_back( fullPass );

            //// Check alpha for glow source
            //osg::TextureRectangle* glowSourceTexture = createTextureRectangle( _screenDims, GL_RGBA );
            //_glowPasses.push_back( checkGlowSourcePass( fullScreenTexture, glowSourceTexture ) );

            // Convolution pass - direction = x
            osg::TextureRectangle* convo_x = createTextureRectangle( _screenDims, GL_RGBA );
            _glowPasses.push_back( glowConvolutionPass( fullScreenGlow, convo_x, osg::Vec2(1.0, 0.0), 3 ) );

            // Convolution pass - direction = y
            osg::TextureRectangle* convo_y = createTextureRectangle( _screenDims, GL_RGBA );
            _glowPasses.push_back( glowConvolutionPass( convo_x, convo_y, osg::Vec2(0.0, 1.0), 3 ) );

            // Convolution pass - direction = x
            osg::TextureRectangle* convo_x2 = createTextureRectangle( _screenDims, GL_RGBA );
            _glowPasses.push_back( glowConvolutionPass( fullScreenGlow, convo_x2, osg::Vec2(1.0, 0.0), 20 ) );

            // Convolution pass - direction = y
            osg::TextureRectangle* convo_y2 = createTextureRectangle( _screenDims, GL_RGBA );
            _glowPasses.push_back( glowConvolutionPass( convo_x2, convo_y2, osg::Vec2(0.0, 1.0), 20 ) );

            // Final pass - combine glow and blend into scene.
            _glowPasses.push_back( glowCombinerPass(fullScreenTexture, convo_y, convo_y2) );
            //_glowPasses.push_back( glowCombinerPass(fullScreenGlow, convo_y, convo_y2) );
...
void OceanScene::preRenderCull( osgUtil::CullVisitor& cv, bool eyeAboveWater, bool surfaceVisible )
{
    ...
    if( eyeAboveWater )
    {
        ...
        {
            // set view and projection to match main camera
            _glowPasses.at(0)->setViewMatrix( currentCamera->getViewMatrix() );
            _glowPasses.at(0)->setProjectionMatrix( currentCamera->getProjectionMatrix() );

            for( unsigned int i=0; i<_glowPasses.size()-1; ++i )
            {
                _glowPasses.at(i)->accept(cv);
            }
        }
        //-- BAWE-20110803: GLOW_SHADER
    ...
    switch(ea.getEventType())
    {
        case(osgGA::GUIEventAdapter::KEYUP):
        {
            ...
            if (ea.getKey() == 'g')
            {
                ...
                _oceanScene->enableGlow(!_oceanScene->isGlareEnabled());
...
//++ BAWE-20110802: GLOW_SHADER
#include <osgOcean/shaders/bawe_glow_checksource_vert.inl>
#include <osgOcean/shaders/bawe_glow_checksource_frag.inl>

osg::Camera* OceanScene::checkGlowSourcePass( osg::TextureRectangle* inputTexture,                                            
                                              osg::TextureRectangle* outputTexture
                                            )
{
    static const char bawe_glow_checksource_vert_file[]       = "bawe_glow_checksource.vert";
    static const char bawe_glow_checksource_frag_file[]       = "bawe_glow_checksource.frag";


    osg::StateSet* ss = new osg::StateSet;

    ss->setAttributeAndModes( 
        ShaderManager::instance().createProgram("glowCheckSource", 
                                                 bawe_glow_checksource_vert_file,  bawe_glow_checksource_frag_file,
                                                 bawe_glow_checksource_vert,       bawe_glow_checksource_frag ), 
                                                 osg::StateAttribute::ON );

    ss->setTextureAttributeAndModes( 0, inputTexture, osg::StateAttribute::ON );
    ss->addUniform( new osg::Uniform( "osgOcean_ColorTexture", 0 ) );

    osg::Geode* fullSizedQuad = createScreenQuad( _screenDims, _screenDims );
    fullSizedQuad->setStateSet(ss);

    osg::Camera* RTTCamera = renderToTexturePass( outputTexture );
    RTTCamera->setProjectionMatrixAsOrtho( 0, _screenDims.x(), 0, _screenDims.y(), 1, 10 );
    RTTCamera->setViewMatrix(osg::Matrix::identity());
    RTTCamera->addChild( fullSizedQuad );

    return RTTCamera;
}

#include <osgOcean/shaders/bawe_glow_combiner_vert.inl>
#include <osgOcean/shaders/bawe_glow_combiner_frag.inl>

osg::Camera* OceanScene::glowCombinerPass( osg::TextureRectangle* fullscreenTexture,
                                           osg::TextureRectangle* glowTexture,
                                           osg::TextureRectangle* glowTexture2
                                          )
{
    osg::Camera* camera = new osg::Camera;
    ...
    static const char bawe_glow_combiner_vert_file[] = "bawe_glow_combiner.vert";
    static const char bawe_glow_combiner_frag_file[] = "bawe_glow_combiner.frag";

    osg::Program* program = 
        ShaderManager::instance().createProgram( "glow_combiner", 
                                                 bawe_glow_combiner_vert_file, bawe_glow_combiner_frag_file, 
                                                 bawe_glow_combiner_vert,      bawe_glow_combiner_frag );

    osg::StateSet* ss = quad->getOrCreateStateSet();
    ss->setAttributeAndModes(program, osg::StateAttribute::ON);
    ss->setTextureAttributeAndModes(0, fullscreenTexture, osg::StateAttribute::ON );
    ss->setTextureAttributeAndModes(1, glowTexture, osg::StateAttribute::ON );
    ss->setTextureAttributeAndModes(2, glowTexture2, osg::StateAttribute::ON );
    ss->addUniform( new osg::Uniform("osgOcean_ColorBuffer",   0 ) );
    ss->addUniform( new osg::Uniform("osgOcean_GlowBuffer", 1 ) );
    ss->addUniform( new osg::Uniform("osgOcean_GlowBuffer2", 2 ) );
...
#include <osgOcean/shaders/bawe_glow_convolution_vert.inl>
#include <osgOcean/shaders/bawe_glow_convolution_frag.inl>

osg::Camera* OceanScene::glowConvolutionPass(osg::TextureRectangle* glowInput, 
                                             osg::TextureRectangle* glowOutput, 
                                             osg::Vec2f direction,
                                             float sample
                                             )
{
    static const char bawe_glow_convolution_vert_file[] = "bawe_glow_convolution.vert";
    static const char bawe_glow_convolution_frag_file[] = "bawe_glow_convolution.frag";

    osg::Camera* glowPass = renderToTexturePass( glowOutput );
    glowPass->setClearColor( osg::Vec4f( 0.f, 0.f, 0.f, 0.f) );
    glowPass->setProjectionMatrixAsOrtho( 0, _screenDims.x(), 0.f, _screenDims.y(), 1.0, 500.f );
    glowPass->setRenderTargetImplementation( osg::Camera::FRAME_BUFFER_OBJECT );
    {
        osg::Program* program = 
            ShaderManager::instance().createProgram( "glow_convo", 
                                                     bawe_glow_convolution_vert_file, bawe_glow_convolution_frag_file, 
                                                     bawe_glow_convolution_vert,      bawe_glow_convolution_frag );

        osg::Geode* screenQuad = createScreenQuad(_screenDims, _screenDims);
        screenQuad->getOrCreateStateSet()->setMode(GL_LIGHTING, osg::StateAttribute::OFF);
        screenQuad->getOrCreateStateSet()->setAttributeAndModes(program, osg::StateAttribute::ON );
        screenQuad->getStateSet()->addUniform( new osg::Uniform("osgOcean_ConvTexture", 0) );
        screenQuad->getStateSet()->addUniform( new osg::Uniform("direction", direction) );
        screenQuad->getStateSet()->addUniform( new osg::Uniform("sample", sample) );
        screenQuad->getOrCreateStateSet()->setTextureAttributeAndModes(0,glowInput,osg::StateAttribute::ON);
        glowPass->addChild( screenQuad ); 
    }
