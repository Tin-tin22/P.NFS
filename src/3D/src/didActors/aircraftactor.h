#ifndef APP_AIRCRAFT_ACTOR
#define APP_AIRCRAFT_ACTOR

#include "didactorsheaders.h"
#include "shipactor.h"

class DID_ACTORS_EXPORT AircraftActor : public VehicleActor
{ 
public:
	AircraftActor(dtGame::GameActorProxy &proxy);
...
private:
	...
	void SetSoundFile();  
	void GetShipChild(){};
	void SetShipSmokeAircraft(const int OrderValue);
	void ProcessOrderEvent(const dtGame::Message &message);
};

class DID_ACTORS_EXPORT AircraftActorProxy : public VehicleActorProxy
{
public:
	/// Constructor
	AircraftActorProxy();
	...
	virtual void CreateActor() { SetActor(*new AircraftActor(*this)); }
protected:
	/// Destructor
	virtual ~AircraftActorProxy();
	virtual void OnEnteredWorld();
private:
};
