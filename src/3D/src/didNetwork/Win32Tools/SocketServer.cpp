...
// Modifications  :
//
// $Log: SocketServer.cpp,v $
// Revision 1.5  2008/05/28 07:56:55  cvsuser
// nambah OnConnectionDropped
//
// Revision 1.4  2008/05/26 07:40:23  cvsuser
...
//
// Revision 1.2  2008/04/01 03:34:14  cvsuser
// Sam.UpdateCVS
//
...
void CSocketServer::StartAcceptingConnections(unsigned long addressToListenOn)
{
   if (m_listeningSocket == INVALID_SOCKET)
   {
      //lint -e{1933} call to unqualified virtual function
      ...
      //lint -e{1933} call to unqualified virtual function
      m_listeningSocket = CreateListeningSocket(addressToListenOn, m_port);
...
int CSocketServer::Run()
{
   try
   {
      ...
      while (!m_shutdownEvent.Wait(0))
      {
         ...
         else if ((waitResult == WAIT_OBJECT_0 + 1)&&(isServer))
         {
            // accept connections, client accept nothing
   ...
   const sockaddr_in &address)
{
   ...
   CSocket soc(CreateOutboundSocket(m_address, m_port));
...
void CSocketServer::Connect2Server()
{
	CSocket::InternetAddress localAddress(m_address, m_port);
	m_outboundSocket = Connect(localAddress);
	m_outboundSocket->Read();
}
