...
type
 ...
 public
  ...
  const IMAGES_PATH  = '..\data\images\tdc\';
...
//  Image1.Height := 1000;
  ...
  tdc_image_path := ExtractFilePath(Application.ExeName) + '..\data\images\tdc\';
  ...
  if Screen.MonitorCount=5 then begin
    Image1.Height := 1150;
    ...
    Image1.Height := 764;
    Image1.Width:= Image1.Height;
    fname := tdc_image_path+ 'tdc_LPD2.bmp';
  end;
...
procedure TfrmTDCTengah.img2MouseDown(Sender: TObject;
...
procedure TfrmTDCTengah.img2MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var ix, iy : integer;
RollBallArea : TRect;
begin
  if rimDown then begin
    RollBallArea := Rect(Point(0,0), Point(img1.height , img1.width));
    if not TestHitView(X, Y, RollBallArea) then Exit;

    ix := X - rimLastPos.X;
    iy := Y - rimLastPos.Y;
    TDCInterface.OBMRight_SetPosition(
        TDCInterface.OBMRight.Center.X + Floor(ix * rimScale),
        TDCInterface.OBMRight.Center.Y + Floor(iy * rimScale));
...
procedure TfrmTDCTengah.img2MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  rimDown   := false;
...
procedure TfrmTDCTengah.img1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  limDown   := true;
  limLastPos.X:= X;
...
procedure TfrmTDCTengah.img1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var ix, iy : integer;
    RollBallArea : TRect;
begin
  if limDown then begin
    ...
    ix := X - limLastPos.X;
    iy := Y - limLastPos.Y;
    TDCInterface.OBMleft_SetPosition(
        TDCInterface.OBMLeft.Center.X + Floor(ix * limScale),
        TDCInterface.OBMLeft.Center.Y + Floor(iy * limScale));
...
procedure TfrmTDCTengah.img1MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  limDown   := false;
