...
uses
  ...
  Dialogs, ufQEK, ufKeyBoard, ImgList, ufANDUDisplay, uLibTDCClass, Buttons,
  SpeedButtonImage, ExtCtrls;
...
type
  ...
  protected
    PCmd : array [1..16]  of TLineCommand;
...
uses
  ...
  uLibTDCTracks, uTorpedoTrack, uTrackFunction, uLibTDC_Nala ;
...
//    result := -1;
  ...
  if ss[0] = 'PAR+' then  begin
     ...
     case ss[1][1] of
        ...
        'C' : begin
          CParLine := i;
          (TDCInterface as TTDC_NalaInterface).CPA_show(PositionID, Display,  CParLine);
  ...
  if ss[0] = 'PAR-' then  begin
     ...
     case ss[1][1] of
        ...
        'C' : begin
          ...
          CParLine := -1;
          (TDCInterface as TTDC_NalaInterface).CPA_show(PositionID, nil,  0);
...
//    result := -9;
  ...
  if ss[0] = 'LIN-' then  begin
    result := -10;
    if IsValidLineNumber(ss[1], i) then begin
      Display.SetTextLineNumber(i, '');
      pCmd[i].active := false;
      pCmd[i].ssCmd.Clear;
      pCmd[i].cmdType := 0;
      result := 10;
...
//////////////////////////////////////////////////////////////////////////////
  ...
  if ss[0] = 'LOT+' then  begin
    ...
    if ss.Count = 2 then begin
      //no spasi version
      if Length(ss[1]) > 1 then  // jam
        s := Copy(ss[1], 1, 2);
      if Length(ss[1]) > 3 then  // menit
        p1 := Copy(ss[1], 3, 2);
      if Length(ss[1]) > 5 then  // detik
        p2 := Copy(ss[1], 5, 2);
    end;
    ...
    if ss.Count >= 4 then begin
      s  := ss[1];   // jam menit detik
      p1 := ss[2];
      p2 := ss[3];
    end;

    if not ConvertToInt(s, i)   or (i < 0) or (i   > 23) then exit;
    if not ConvertToInt(p1, i2) or (i < 0) or (i2  > 59) then exit;
    if not ConvertToInt(p2, len)or (i < 0) or (len > 59) then exit;

    dt := i + (i2/60.0) + (len/3600.0);
    TDCInterface.tdcData.LocalTime := dt / 24.0;
...
//    result := -24;
  ...
  if ss[0] = 'LCN+' then  begin
  ...
  else if ss[0] = 'SSM+' then  begin
    if (ss.Count = 2 ) then begin
      if ss[1] = 'RBL' then
        TDCInterface.assssm.LaunchMode := lmRBL
      else if ss[1] = 'BOL' then
          TDCInterface.assssm.LaunchMode := lmBOL
    end;
    ...
    case ss[1][1] of
      'R' : TDCInterface.assssm.RangeToGo    := sz;
      'W' : TDCInterface.assssm.Search_Width := sz;
      'D' : TDCInterface.assssm.Search_Depth := sz;
    end;

  end
  else
  {# 57}
  if ss[0] = 'CPA-' then  begin
     (TDCInterface as TTDC_NalaInterface).CPA_start(PositionID, nil);
  end
  else
  if ss[0] = 'SCR-' then  begin
    if IsValidLineNumber(ss[1], i) then begin
      Display.SetTextLineNumber(i, '');
      pCmd[i].active := false;
      pCmd[i].ssCmd.Clear;
...
//    result := -41;
  ...
  if ss[1] = 'TLI+' then  begin
    if ss.Count < 3 then exit;
    if (cTrack is TRefPosTrack) and IsValidTwoLetterIndication(ss[2]) then begin
  ...
  if ss[1] = 'CPA+' then  begin
     if (mTrack = nil) then exit;
      (TDCInterface as TTDC_NalaInterface).CPA_start(PositionID, mTrack);
      result := 56;
  end
  else
  {# 58}
...
var ss : TStrings;
    s, sCmd, sParam : string;
    i, m, l: integer;

begin
  sCmd := Trim(str);
  if Length(sCmd) < 4 then exit; // tidak ada command < 4 char.

  if Length(sCmd) = 4 then begin // non track action


  end;

  if Length(sCmd) > 4 then begin //

    s := Copy(sCmd, 1, 4);
    if IsValidTrackNumber(s) and (sCmd [5] <> ' ') then  Insert(' ', sCmd, 5);
    m := 1;
    l := Length(sCmd);
    i := PosEx('+', sCmd, m );
    while  (i > 1) do begin
  ...
  if ss.Count > 1 then begin
    if ss[0] = 'LOAD' then begin
      ...
      if IsValidTrackNumber(ss[0]) then begin
        if (ss.Count > 2) and (ss[1] = 'SCR+') then begin
          if IsValidLineNumber(ss[2], i) then begin

            pCmd[i].ssCmd.Clear;
            pCmd[i].ssCmd.AddStrings(ss);
            pCmd[i].cmdType := 1; // track action
            pCmd[i].active := true;
          end;
        end;
...
{SLL+}
        ...
        if (ss[0] = 'SLL+') then begin
          if IsValidLineNumber(ss[1], i) then begin
            pCmd[i].ssCmd.Clear;
            pCmd[i].ssCmd.AddStrings(ss);
            pCmd[i].cmdType := 2; // non track action
            pCmd[i].active := true;
          end;
        end;
    ...
    if (ss[0] = 'QGC+') or
       ...
       pCmd[15].ssCmd.Clear;
       pCmd[15].cmdType := 2; // non track action
       pCmd[15].ssCmd.AddStrings(ss);
       pCmd[15].active := true;
    end;
...
begin
  ...
  for i := 1 to 15 do begin
    pCmd[i].active := false;
    pCmd[i].cmdType := 0;
    pCmd[i].ssCmd := TStringList.Create;
  end;
end;
...
procedure TfrmANDUKey.FormDestroy(Sender: TObject);
var i: integer;
begin
  for i := 1 to 15 do begin
    pCmd[i].active := false;
...
procedure TfrmANDUKey.UpdateDisplayData;
var i: integer;
begin
  for i := 1 to 15 do begin
    if pCmd[i].active then begin

      case pCmd[i].cmdType of
       1 : Do_Track_Action(pCmd[i].ssCmd);
       2 : Do_NonTrack_Action(pCmd[i].ssCmd)
      end;
    end;
  end;
end;
