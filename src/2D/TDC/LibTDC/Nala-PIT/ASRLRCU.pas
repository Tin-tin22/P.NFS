...
uses
  ...
  VrDesign, uLibTDCClass, Menus, ufLog;
...
begin
  ...
  if btnFE1.down then begin
    ...
    if Assigned(frmLog) then frmLog.AddLog('btFe1 is down');
  end
  else if btnFE2.down then begin
    Launcher:= 2;
    IslauncherSelected := True;
    if Assigned(frmLog) then frmLog.AddLog('btnFe2 is down');
  end;
...
//  Spd:= GetRocketSpeed(Rocket);
  ...
  begin
    if CekReadyCondition then
      frmLog.AddLog('CekReadyCondition is true')
    else
      frmLog.AddLog('CekReadyCondition is false');

    if IslauncherSelected then
      frmLog.AddLog('LauncherSelected is true')
    else
      frmLog.AddLog('LauncherSelected is false');
  end;
...
procedure TfrmASRL.Hide1Click(Sender: TObject);
begin
  frmLog.Hide;
end;
...
procedure TfrmASRL.Log1Click(Sender: TObject);
begin
 frmLog.Show;
end;
...
begin
  ...
  MaxLim := 0;
      ...
      case rockettype  of
        1 : begin
          if (aRange >= C_RocketErika_LOW_Min) and (aRange <= C_RocketErika_LOW_Max) then
            begin
              if Assigned(frmLog) then frmLog.AddLog('rocket type 1 between erika low min and erika high max');
        ...
        2: begin
          if (aRange >= C_RocketErika_HIGH_Min) and (aRange <= C_RocketErika_HIGH_Max) then
            begin
              if Assigned(frmLog) then frmLog.AddLog('rocket type 2 between erika high min and erika high max');
        ...
        3 : begin
          if (aRange >= C_RocketNelly_LOW_Min) and (aRange <= C_RocketNelly_LOW_Max) then
            begin
              if Assigned(frmLog) then frmLog.AddLog('rocket type 3 between nelly low min and nelly low max');
        ...
        4: begin
          if (aRange >= C_RocketNelly_HIGH_Min) and (aRange <= C_RocketNelly_HIGH_Max) then
            begin
              if Assigned(frmLog) then frmLog.AddLog('rocket type 4 between nelly high min and nelly high max');
   ...
   begin
     ...
     if Assigned(frmLog) then frmLog.AddLog('Out of Firing angle');
   end;
...
//    loadimage(imgBLDARC,  'BLD ARC on.bmp');
    if Assigned(frmLog) then frmLog.AddLog('CekBlindArc is true');
    Result := True;
    end else begin
//    loadimage(imgBLDARC,  'BLD ARC off.bmp');
    if Assigned(frmLog) then frmLog.AddLog('CekBlindArc is false');
    BldArc.Status := false;
   end;
...
begin
 ...
 if not (TDC as TTDC_NalaInterface).IsASRLAssigned then begin
   ...
   tmr1.Enabled := false;
   if Assigned(frmLog) then
     frmLog.AddLog('ASSRL is not Assigned');
 end
 else begin
   (TDC as TTDC_NalaInterface).GetCurrentDataTrackASRL(OwnHeading,Target_Range,Target_Bearing);
   if Assigned(frmLog) then
   begin
     frmLog.AddLog('ASSRL is Assigned');
     frmLog.AddLog('Own heading : ' + FloatToStr(OwnHeading));
     frmLog.AddLog('Target range : ' + FloatToStr(Target_Range));
     frmLog.AddLog('Target bearing : ' + FloatToStr(Target_Bearing));
   end;

   if IsRocketSelected then begin
    if Assigned(frmLog) then frmLog.AddLog('Rocket is selected');
      ...
      case Rocket  of
        1,2 :begin
           if (Target_Range >= C_RocketErika_LOW_Min) and (Target_Range <= C_RocketErika_LOW_Max) then
              begin
                Msl := 1;
                if Assigned(frmLog) then frmLog.AddLog('Msl 1');
              end
           else  if (Target_Range >= C_RocketErika_HIGH_Min) and (Target_Range <= C_RocketErika_HIGH_Max) then
              begin
                Msl := 2;
                if Assigned(frmLog) then frmLog.AddLog('Msl 2');
              end;
         end;
         3,4 :begin
           if (Target_Range >= C_RocketNelly_LOW_Min) and (Target_Range <= C_RocketNelly_LOW_Max) then
              begin
                Msl := 3;
                if Assigned(frmLog) then frmLog.AddLog('Msl 3');
              end
           else  if (Target_Range >= C_RocketNelly_HIGH_Min) and (Target_Range <= C_RocketNelly_HIGH_Max) then
              begin
                Msl := 4;
     ...
     if (Msl >= 1) and (Msl <= 4) and (Msl <> Rocket) then begin
       ...
       Rocket := Msl;
       if Assigned(frmLog) then frmLog.AddLog('Is not Rocket');
       spdInd := Rocket mod 2;
       SetChangeImageIndikator(Low.Imgobj , Low.ImgON, Low.imgOFF, spdInd = 1);
       SetChangeImageIndikator(High.Imgobj , High.ImgON, High.imgOFF, spdInd = 0);
     end;
