...
{     CurrentPageType :   tPageType;
     ...
     procedure SelectPage(pgIndex: byte);
  public
...
begin
  if (line >=1) and (line <= 16) then begin
    ...
    ANDULines[line].Text := InsertSpace(ss);
  end;
end;
...
begin
  if (line >=1) and (line <= 16)
    and (col >= 0) and (col < 32) then begin
...
//      StrCmd := RemoveSpace(AnduTDC16.Text);
...
{ else
  if iKey = VK_LEFT	then begin
    AnduTDC16.SelStart := 0;
    AnduTDC16.SelLength :=  AnduTDC16.SelLength-1;
  end
}
{ VK_UP	Up Arrow key
